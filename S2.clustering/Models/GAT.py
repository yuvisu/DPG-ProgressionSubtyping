...
if __name__ == "__main__":
    ...
    hidden_dim = 16
    output_embedding_dim = 16 # embedding size will be head * output_embedding_dim
    heads = 8
    num_hidden_layers = 2
    dropout_prob = 0.7
    epochs = 100
    ...
    # loading model and trainning
    GAT = GAT(input_dim = input_dim, hidden_dim = hidden_dim, output_embedding_dim = output_embedding_dim, output_dim = output_dim, heads= heads,
                          num_layers = num_hidden_layers, dropout_prob = dropout_prob)
    GAT.to(device)
    ...
    if optimizer == 'Adam':
        ...
        optimizer = torch.optim.SGD(GAT.parameters(), lr=learning_rate, weight_decay=weight_decay)
    else:
        optimizer = torch.optim.Adam(GAT.parameters(), lr=learning_rate, weight_decay=weight_decay)

    #Trainning
    GAT.fit(train_data, val_data, epochs=epochs, optimizer=optimizer)

    output_from_model = GAT(data.x, data.edge_index)
    embedding_feature = output_from_model[0].cpu().detach().numpy()
    print(embedding_feature.shape)
    print(accuracy(output_from_model[1].argmax(dim=1), data.y))
    """