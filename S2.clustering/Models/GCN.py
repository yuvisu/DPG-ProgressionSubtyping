...
if __name__ == "__main__":
    ...
    GCNmodel = GCN_model(input_dim = input_dim, hidden_dim = hidden_dim,output_embedding_dim = output_embedding_dim,  output_dim = output_dim,
                          num_layers = num_GCN_layers, dropout_prob = dropout_prob)
    GCNmodel.to(device)

    # Define the optimizers to use
    if optimizer == 'Adam':
        optimizer = torch.optim.Adam(GCNmodel.parameters(), lr=learning_rate, weight_decay=weight_decay)
    elif optimizer == 'Sgd':
        optimizer = torch.optim.SGD(GCNmodel.parameters(), lr=learning_rate, weight_decay=weight_decay)
    else:
        optimizer = torch.optim.Adam(GCNmodel.parameters(), lr=learning_rate, weight_decay=weight_decay)

    GCNmodel.fit(train_data, val_data, epochs=epochs, optimizer=optimizer)


    output_from_model = GCNmodel(data.x, data.edge_index, data.edge_weight)
